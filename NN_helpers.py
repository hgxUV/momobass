...
def draw_response3D(X,Y, func):
    ...
    ax.scatter(X[:, 0], X[:, 1], Y, c=Y, cmap='coolwarm', edgecolors='k', s=80)
    plt.show()


def draw_response3D_pytorch(X, Y, net):
    h = .02  # step size in the mesh
    X = np.array(X)  # only needed if someone provided a list of lists as X
    x_min, x_max = X[:, 0].min() - .5, X[:, 0].max() + .5
    y_min, y_max = X[:, 1].min() - .5, X[:, 1].max() + .5
    ...
    x = torch.from_numpy(xx.reshape(-1, 1))
    y = torch.from_numpy(yy.reshape(-1, 1))
